...
func TestAdminCommands(t *testing.T) {
	...
	// 1. /shutdown test (even with invisible marks)
	res := HandleAdminCommand(state, chatID, sender, "Member", false, "\u200e/shutdown", stats)
	...
	// 2. /start test
	res = HandleAdminCommand(state, chatID, sender, "Member", false, "/start", stats)
	if !res.Handled || state.GetShutdown() {
		t.Errorf("expected /start to reset shutdown mode")
	}

	// 3. /set test
	res = HandleAdminCommand(state, chatID, sender, "Member", false, "/set 500", stats)
	if !res.Handled || state.GetChatLimit(chatID, 0) != 500 {
		t.Errorf("expected chat limit 500, got %d", state.GetChatLimit(chatID, 0))
	}

	res = HandleAdminCommand(state, chatID, sender, "Member", false, "/set all", stats)
	if !res.Handled || state.GetChatLimit(chatID, 100) != 0 {
		t.Errorf("expected chat limit 0 (all), got %d", state.GetChatLimit(chatID, 100))
	}

	// 4. /auto test
	res = HandleAdminCommand(state, chatID, sender, "Member", false, "/auto on", stats)
	if !res.Handled || !state.IsAutoTriggersEnabled(chatID) {
		t.Errorf("expected auto triggers enabled")
	}

	res = HandleAdminCommand(state, chatID, sender, "Member", false, "/auto off", stats)
	if !res.Handled || state.IsAutoTriggersEnabled(chatID) {
		t.Errorf("expected auto triggers disabled")
	}

	// 5. /status test (with invisible LTR mark \u200e)
	res = HandleAdminCommand(state, chatID, sender, "Member", false, "\u200e/status", stats)
